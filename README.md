...
1. **Ingestion Data**: Mengambil data dinamis langsung dari API Open Data Jawa Barat dengan limit 1000 record.
...
6. **Optimasi Hyperparameter**: Evaluasi jumlah klaster optimal ($K$) menggunakan **Elbow Method** (Inersia) dan **Silhouette Score**.
7. **Modelling & Labeling**: Implementasi algoritma K-Means, lalu cluster diurutkan berdasarkan volume kunjungan tahunan untuk dilabeli menjadi **Potensi Rendah**, **Potensi Sedang**, dan **Potensi Tinggi**.
8. **Persistensi Model**: Menyimpan model terlatih dan scaler ke folder `models/` menggunakan `joblib`.
9. **Dashboard Visualization**: Menyajikan hasil analisis dalam antarmuka interaktif Streamlit multi-halaman.
...
### 4. Jalankan Pipeline Modeling (Opsional - Data diunduh & dilatih otomatis oleh Streamlit)
