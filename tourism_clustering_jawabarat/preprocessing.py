...
def clean_data() -> pd.DataFrame:
    ...
    This function requires the local raw dataset file to exist.
    It does NOT call any external API. If the file is missing,
    a FileNotFoundError is raised with a clear diagnostic message.
    """
    # --- Diagnostic logging ---
    import os
    logging.info(f"[Preprocessing] Current working directory: {os.getcwd()}")
    logging.info(f"[Preprocessing] Dataset path: {RAW_DATA_PATH}")
    logging.info(f"[Preprocessing] Dataset exists: {RAW_DATA_PATH.exists()}")

    if not RAW_DATA_PATH.exists():
        raise FileNotFoundError(
            f"Raw dataset file not found at: {RAW_DATA_PATH}\n"
            f"Current working directory: {os.getcwd()}\n"
            f"Please ensure 'data/raw/raw_tourism_data.json' is committed to the repository "
            f"and not excluded by .gitignore."
        )
    ...
    if "data" not in raw_json or not raw_json["data"]:
        raise ValueError(
            f"Raw JSON file at {RAW_DATA_PATH} does not contain a valid 'data' key, "
            f"or the 'data' array is empty."
        )

    df = pd.DataFrame(raw_json["data"])
    logging.info(f"[Preprocessing] Rows loaded from raw JSON: {len(df)}")

    if df.empty:
        raise ValueError(
            "Loaded DataFrame is empty after reading raw JSON. "
            "Check the contents of the raw dataset file."
        )
    ...
    logging.info(f"[Preprocessing] Rows after preprocessing: {df.shape[0]}")
